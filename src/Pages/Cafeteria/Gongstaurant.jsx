// FE2
// 공식당 페이지
// src/Pages/Cafeteria/Gongstaurant.jsx
...
const CAFETERIA_ID = "gong"; // 백엔드에서 쓸 공식당 id로 바꾸기
...
function getLastWeekSentence(history) {
  ...
  // 여기서는 가장 마지막 데이터 기준으로 문장 생성
  const last = history[history.length - 1];

  const timeLabel = last.time || "해당 시간";
  const levelLabel = levelToKorean(last.level);

  return `일주일 전 ${timeLabel}에는 공식당이 ${levelLabel}했어요.`;
}

export default function Gongstaurant() {
  ...
  const handleVote = useCallback(
    async (level) => {
      try {
        ...
        // 사용자가 "여유"에 투표한 경우에도 럭키 모달 한 번 띄워주기
        if (level === "relaxed" && !luckyShownOnce) {
          setShowLucky(true);
          setLuckyShownOnce(true);
        }

        // 옵션: 투표 후에 상태/히스토리 다시 불러오고 싶으면 여기서 load 호출하면 됨
      } catch (err) {
        console.error(err);
